...
def evaluate_retrieval(queries, qrels, corpus, true_answers, tokenizer, model, device, randomize_order):
    ...
    max_k = metrics.top_k
    all_responses = contrvr_search.retrieve(corpus, queries, max_k, similarity_measure, chunk=True, chunksize=500)
    ...
    for k in metrics.k_values:
        filtered_responses = {
            ...
            } for query_id, docs in all_responses.items()
        }
        ...
        logging.info(f"Score for top-{k} contexts: {avg_em_score}")
